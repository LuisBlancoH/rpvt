"""Experiment v3.12: Brain-inspired memory decoder for instruct models.

A dedicated transformer decoder reads from memory and produces rich
context tokens prepended to the main model's input. The main model
is 100% frozen — generation quality preserved by construction.

Unlike soft prompts (8 learned vectors → 33% recall), the decoder
is a full 2-layer transformer with cross-attention to memory,
producing 32 rich context tokens. This should carry much more
information from memory to the model.

Architecture:
  Layer 14 (WriteWrapper): writes to MemoryBank (same as always)
  At query time:
    MemoryDecoder: memory → 2-layer transformer → 32 context tokens
...
from rpvt.model.memory_decoder import MemoryDecoder
...
def train(model, decoder, memory_bank, tokenizer, train_docs, eval_docs,
          device, num_epochs=15, lr=1e-3, log_every=50, checkpoint_dir=None):
    ...
    for epoch in range(num_epochs):
        ...
        for doc_idx in order:
            ...
            # Decode memory into context tokens
            mem_states, n_active = memory_bank.get_active_memories()
            context = decoder(mem_states, n_active)  # (1, n_tokens, hidden)

            # Forward QA chunk with memory context
            qa_chunk = chunks[-1].unsqueeze(0).to(device)
            output = forward_with_context(model, qa_chunk, context, embed_fn)
        ...
        eval_results = evaluate(model, decoder, memory_bank, tokenizer,
    ...
    return evaluate(model, decoder, memory_bank, tokenizer, eval_docs,
                    device, embed_fn)


def evaluate(model, decoder, memory_bank, tokenizer, eval_docs, device, embed_fn):
    model.eval()
    decoder.eval()
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for doc in eval_docs:
            ...
            mem_states, n_active = memory_bank.get_active_memories()
            context = decoder(mem_states, n_active)
...
def test_generation(model, decoder, memory_bank, tokenizer, device):
    model.eval()
    decoder.eval()
    embed_fn = get_embeddings_fn(model)
        ...
        with torch.no_grad():
            ...
            if context is not None:
                ...
                inputs_embeds = torch.cat([
                    context.expand(1, -1, -1), token_embeds
                ], dim=1)
...
def main():
    ...
    parser.add_argument("--output-dir", default="results/exp_v3_12_decoder")
    ...
    results = train(
        model, decoder, memory_bank, tokenizer, train_docs, eval_docs,
    ...
    test_generation(model, decoder, memory_bank, tokenizer, args.device)
