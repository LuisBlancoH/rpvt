"""v3.29: Train Predictive Transformer on Instruct model + QA.

Qwen2.5-0.5B-Instruct (frozen) + trainable mechanisms.
Train on mix of:
  - WikiText (general LM, maintains fluency)
  - Synthetic QA (teaches memory retrieval)

Passage stored in memory → question answered from memory.
Clear gradient signal: correct answer = memory worked.

Usage:
    python -m rpvt.experiments.exp_v3_29_instruct_qa
    python -m rpvt.experiments.exp_v3_29_instruct_qa --epochs 20
"""
...
def generate_qa_sample(rng):
    """Generate a single QA sample with passage and question."""
    ...
    answer = a_template.format(
        name=name, company=company, city=city, role=role
    )
...
class QADataset(Dataset):
    """Generates QA pairs on the fly."""

    def __init__(self, tokenizer, n_samples=1000, seed=42):
    ...
    def __getitem__(self, idx):
        ...
        # Passage tokens (chunk 1 — stored in memory)
        passage_ids = self.tokenizer.encode(
            sample["passage"], add_special_tokens=False
        )

        # Question + answer tokens (chunk 2 — uses memory)
...
class WikiTextDataset(Dataset):
    """WikiText chunks for general LM training."""
...
def train_qa_batch(model, sample, device):
    """Train on one QA sample: passage → memory → question → answer."""
    model.reset_state()

    # Chunk 1: passage (store in memory, no loss)
    p_ids = sample["passage_ids"].unsqueeze(0).to(device)
    model.forward(p_ids, n_settle=1)
    model.detach_state()

    # Chunk 2: QA (use memory, compute loss on answer tokens)
    qa_ids = sample["qa_ids"].unsqueeze(0).to(device)

    # Find where the answer starts (after "A: ")
    qa_text_tokens = sample["qa_ids"]
    # Create labels: -100 for question part, actual ids for answer part
    # Simple approach: loss on all tokens (question is easy, answer needs memory)
    input_ids = qa_ids[:, :-1]
    labels = qa_ids[:, 1:]

    result = model.forward(input_ids, labels=labels, n_settle=1,
                          return_errors=True)
    loss = result[1]
    return loss
...
def train_wiki_batch(model, input_ids, targets, device):
    """Train on WikiText batch (general LM)."""
    model.reset_state()
    input_ids = input_ids.to(device)
    targets = targets.to(device)
    result = model.forward(input_ids, labels=targets, n_settle=1,
                          return_errors=True)
    return result[1]


def evaluate_qa(model, tokenizer, device, n=50, seed=99):
    """Evaluate QA accuracy: passage → memory → generate answer."""
    model.eval()
    ...
    for _ in range(n):
        ...
        with torch.no_grad():
            gen = model.generate(q_ids, max_new_tokens=15, n_settle=1)
...
def evaluate_qa_no_memory(model, tokenizer, device, n=50, seed=99):
    """Evaluate QA without memory (baseline)."""
    model.eval()
    rng = random.Random(seed)
    correct = 0
    total = 0
    ...
    for _ in range(n):
        ...
        # Skip passage — just ask question
        q_text = f"Q: {sample['question']} A:"
        q_ids = tokenizer(q_text, return_tensors="pt").input_ids.to(device)
        with torch.no_grad():
            gen = model.generate(q_ids, max_new_tokens=15, n_settle=1)
...
def main():
    ...
    parser.add_argument("--n-qa", type=int, default=2000,
                        help="QA samples per epoch")
    parser.add_argument("--wiki-ratio", type=float, default=0.3,
                        help="Fraction of batches that are WikiText (rest are QA)")
    parser.add_argument("--grad-accum", type=int, default=8)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--output-dir", type=str,
                        default="results/instruct_qa")
    ...
    for epoch in range(args.epochs):
        ...
        for qa_idx in qa_indices:
            ...
            # Wiki batch (with probability wiki_ratio)
            ...
            if step % 200 == 0:
                ...
                print(f"    [{step}/{len(qa_indices)}] "
                      ...
                      f"mem={model.memory.n_stored}/{model.memory.n_slots} "
