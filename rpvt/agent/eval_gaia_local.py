"""GAIA evaluation with local Qwen3.5-9B agent.

Usage:
    python -m rpvt.agent.eval_gaia_local
    python -m rpvt.agent.eval_gaia_local --max-tasks 15
    python -m rpvt.agent.eval_gaia_local --no-code
"""
...
def main():
    ...
    # Initialize agent
    from rpvt.agent.local_agent import LocalAgent
    agent = LocalAgent(
    ...
    print(f"\n{'='*60}")
    print(f"GAIA Evaluation: {len(tasks)} tasks")
    print(f"{'='*60}")
